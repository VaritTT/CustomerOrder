...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $sql = "SELECT * FROM order_header WHERE CONCAT(order_id,customer_id,order_address) LIKE '%$search%' ORDER BY order_id";
} else if (isset($_SESSION['periods_sql'])) {
    if (isset($_SESSION['periods_name'])) {
        $periods_name = $_SESSION['periods_name'];
    }
    $sql = $_SESSION['periods_sql'] . " ORDER BY order_datetime DESC";
} else {
...
<body>
    ...
    <div class="col-12 mt-5 mb-4 text-center">
        <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
            <a href="report_order_all.php" class="btn btn-outline-info active">ทั้งหมด</a>
            <a href="report_order_paid.php" class="btn btn-outline-info">ชำระเงินแล้ว</a>
    ...
    <div class="d-flex justify-content-between mt-4">
        <div class="d-flex">
            <h4 style="font-weight: 600;">รายงานการสั่งซื้อ(ทั้งหมด) <?php echo $periods_name; ?> <span class="badge text-bg-info"><?php echo $row_count . "/" . $orderAll; ?></span></h4>
        ...
        <div class="text-end">
            <?php if (isset($_SESSION['periods_name'])) { ?>
                <a href="report_order_yes.php?period=<?= $periods_name ?>"><button class="btn btn-warning" type="button">Export PDF</button></a>
            <?php } else { ?>
                <a href="report_order_yes.php?"><button class="btn btn-warning" type="button">Export PDF</button></a>
            <?php } ?>
        </div>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td class="col-2 text-center" style="vertical-align: middle;"><?= $row['order_address'] ?></td>
